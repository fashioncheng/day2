...
// 多曲操作
function multipleMusicOption(option,musicString){
...
$(".m_down").live("click",function(){
    ...
    var playMusicString = MUSICLISTOBJ[rid];
    if(rid&&playMusicString){
        singleMusicOption("Down",playMusicString+"&searchlog="+sendFromSearchLog("operation",logObj,"needResult"));
    }
});
$(".m_more").live("click",function(){
    ...
    var playMusicString = MUSICLISTOBJ[rid];
    if(rid&&playMusicString){
        var call = "ShowOperation?song="+playMusicString+searchLog;
        callClientNoReturn(call);
    }
    ...
    try{
        $(".newm_more").live("click",function(){
            ...
            if(bOk){
                ...
                //主要展示的逻辑
                if($(openArea).attr('data-flag')==1){
                    $('.openArea').attr('data-flag',0);
                    $(openArea).attr('data-flag',1);
                }else{
                    $('.openArea').attr('data-flag',0);
                }
                if($(openArea).find('.openSong').html()=='' && $(openArea).find('.album').html()==''){
                    ...
                    var URL='http://rcm.kuwo.cn/rec.s?cmd=rcm_sim_pl&uid='+getUserID('uid')+'&devid='+getUserID('devid')+'&platform=pc&rid='+$(this).attr('data-rid')+'&plnum=15&musicnum=15';
                    $.ajax({
                        url:URL,
                        dataType:'jsonp',
                        crossDomain:false,
