...
function checkSearchServer(type,key){
	...
	var url = eval('dns'+Math.round(Math.random()+1));
	$.ajax({ 
		type: "get", 
		url: url, 
		cache: false, 
...
function searchDomainChange(key,type,agent){
	agentFlag = true;
	var rn = 100;
	var url = 'http://'+hostConfig+'/r.s?all='+key+'&ft=music&newsearch=1&alflac=1&itemset=web_2013&client=kt&cluster=0&pn='+pn+'&rn='+rn+'&vermerge=1&rformat=json&encoding=utf8&show_copyright_off=1&pcmp4=1&thost=search.kuwo.cn';
...
function get_musiclist_data(key){
	...
	var url = host_url+'r.s?all='+key+'&ft=music&newsearch=1&alflac=1&itemset=web_2013&client=kt&cluster=0&pn='+pn+'&rn='+rn+'&vermerge=1&rformat=json&encoding=utf8&show_copyright_off=1&pcmp4=1';
...
function get_mv_data(key){
	...
	var url = host_url+"r.s?all="+key+"&ft=music&newsearch=1&itemset=web_2013&client=kt&cluster=0&pn="+pn+"&rn="+rn+"&rformat=json&hasmkv=1&encoding=utf8&show_copyright_off=1&pcmp4=1";
	url = getChargeURL(url);
	$.ajax({
        url:url,
		dataType:"text",
...
function get_playlist_data(key){
	var rn = 100;
	if(agentFlag){
		searchDomainChange(key,"setlist",'/r.s?all='+key+'&pn='+pn+'&rn='+rn+'&ft=playlist&encoding=utf8&rformat=json&pay=0&needliveshow=0' + '&plat=pc&devid=' + getUserID("devid"));
		return;
	}
	var url = host_url +'r.s?all='+key+'&pn='+pn+'&rn='+rn+'&ft=playlist&encoding=utf8&rformat=json&pay=0&needliveshow=0';
	$.ajax({
        url:getChargeURL(url),
		dataType:"text",
		type:"get",
...
function get_radio_data(key){
	var rn = 100;
	if(agentFlag){
		searchDomainChange(key,"radio",'/r.s?all='+searchKey+'&pn='+pn+'&rn='+rn+'&ft=recordlist&encoding=utf8&rformat=json&pay=0&needliveshow=0' + '&plat=pc&devid=' + getUserID("devid"));
		return;
	}
	var url = host_url +'r.s?all='+searchKey+'&pn='+pn+'&rn='+rn+'&ft=recordlist&encoding=utf8&rformat=json&pay=0&needliveshow=0';
	$.ajax({
        url:getChargeURL(url),
		dataType:"text",
		type:"get",
...
function get_artist_data(key){
	var rn = 100;
	if(agentFlag){
		searchDomainChange(key,"artist","/r.s?all="+key+"&ft=artist&newsearch=1&itemset=artist_2015&client=kt&cluster=0&pn="+pn+"&rn="+rn+"&rformat=json&encoding=utf8" + '&plat=pc&devid=' + getUserID("devid"));
		return;
	}
	var url = host_url+"r.s?all="+key+"&ft=artist&newsearch=1&itemset=artist_2015&client=kt&cluster=0&pn="+pn+"&rn="+rn+"&rformat=json&encoding=utf8&pcmp4=1";
	$.ajax({
        url:getChargeURL(url),
		dataType:"text",
		type:"get",
...
function get_album_data(key){
	...
	var url = host_url+"r.s?all="+key+"&ft=album&newsearch=1&itemset=web_2013&client=kt&cluster=0&pn="+pn+"&rn="+rn+"&rformat=json&encoding=utf8&show_copyright_off=1";
	url = getChargeURL(url);
	$.ajax({
        url:url,
		dataType:"text",
...
function get_lrc_data(key){
	...
	var url = host_url+"r.s?lrccontent="+key+"&ft=music&newsearch=1&alflac=1&itemset=web_2013&client=kt&cluster=0&pn="+pn+"&rn="+rn+"&rformat=json&encoding=utf8&primitive=0&show_copyright_off=1&pcmp4=1";
	url = getChargeURL(url);
	$.ajax({
        url:url,
		dataType:"text",
...
function create_dhj(key){
    ...
    var searchBeginNum = "utf8_" + getUserID("devid") + encodeURIComponent(searchKey.replace("&", "")) + parseInt((nowDate.getTime() + nowDate.getTimezoneOffset() * 60 * 1000 - pastDate.getTime())/1000/60/60);
...
function create_xiu_area(data,searchBeginNum,firstsearcharray){
	if(typeof(data)!="undefined"&&typeof(data.success)=="boolean"&&data.success===true){
        ...
        $("#search_heji_xiu .juneBoxRrightTop").html('<a href="###" hidefocus data-url="'+data.musicPlayLink+'" onclick="openXIU(this,-1);return false;" class="juneListenBtn2">马上去现场></a>');
...
function objBind(){
    ...
    $(".m_more").live("click",function(){
        ...
        var playMusicString = MUSICLISTOBJ[rid];
        if(rid&&playMusicString){
            var call = "ShowOperation?song="+playMusicString+"&searchlog="+sendSearchLog("operation",logObj,"needResult");
		    callClientNoReturn(call);
        }
    });
    // 歌曲名称 点击
    $(".w_name").live("click",function(){
        ...
        var playMusicString = MUSICLISTOBJ[rid];
        if(rid&&playMusicString){
            singleMusicOption("Play",playMusicString,'',name.replace(/<\/?.+?>/g,''));
        }
        // 新搜索统计日志 露出播放
		var $ul = $this.parents("ul");
		var $music_wrap = $this.parents(".music_wrap");
    ...
    $(".music_wrap").live("dblclick", function () {
        ...
        var playMusicString = MUSICLISTOBJ[rid];
        if (rid && playMusicString) {
            singleMusicOption("Play", playMusicString,'',$this.find(".w_name").html().replace(/<\/?.+?>/g,''));
        }
        // 新搜索统计日志 露出播放
		var $ul = $this.parents("ul");
	...
	$(".forLog").live("click",function(){
		...
		if($this.hasClass("m_album")){
			operationType = "TOALBUM";
		}
		var logObj = {
			"operationType":operationType,
			"pos":pos,
			"ref":"乐库->搜索结果->"+searchKey+"->REF单曲->查看歌手",
...
function sendSearchLog(type,logObj,result){
	...
	var snum = uid+enKey;
	var searchNo = uid+searchKey;
	var time = new Date().getTime();
	var logStr = "";
	var mustLogStr = "";
	if(type=="dhj"){
		logStr = "SNUM:"+(snum+parseInt(time/36000))+"|DHJ_TYPE:"+logObj.dhjType+"|DHJ_NAME:"+logObj.dhjName+"|SKEY:"+searchKey+"|SOURCEID:"+logObj.rid+"|HITNUM:"+logObj.hitNum+"|KEY:"+searchKey+"|TIME:"+time+"|SEARCHNO:"+searchNo+time;
	}else if(type=="dhjClick"){
		logStr = "SNUM:"+(snum+parseInt(time/36000))+"|OPERATIONTYPE:COLLECTIONCLICK|DHJ_TYPE:"+logObj.dhjType+"|DHJ_NAME:"+logObj.dhjName+"|SKEY:"+searchKey+"|REF:乐库->搜索结果->"+searchKey+"->REF单曲|SOURCEID:"+logObj.rid+"|POS:0|HITNUM:"+logObj.hitNum+"|KEY:"+searchKey+"|TIME:"+time+"|SEARCHNO:"+searchNo+time;
	}else{
		if(type=="operation"){
			mustLogStr = "|STYPE:TYPE_SOPERATION|OPERATIONTYPE:"+logObj.operationType+"|POS:"+logObj.pos+"|INNERPOS:"+logObj.innerPos+"|HASLYRIC:0|REF:"+logObj.ref+"|SOURCEID:"+logObj.rid;
		}else if(type=="operationRef"){
			mustLogStr = "|STYPE:TYPE_SOPERATION|OPERATIONTYPE:REF|POS:"+logObj.pos+"|SEARCHTYPE:MUSIC|REFADDR:"+logObj.refaddr+"|REF:"+logObj.ref+"|SOURCEID:"+logObj.rid;
		}else if(type=="search"){
			mustLogStr = "|SKEY:"+searchKey+"|SEARCHMODE:"+logObj;
		}
		logStr = "SNUM:"+(snum+parseInt(time/36000))+mustLogStr+"|HITNUM:"+hitNum+"|KEY:"+searchKey+"|TIME:"+time+"|SEARCHNO:"+searchNo+time;
	}
	if(result){
		return logStr;
	}else{
