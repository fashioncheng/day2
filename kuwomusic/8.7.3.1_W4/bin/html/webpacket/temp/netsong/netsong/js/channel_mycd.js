...
function SetPageIconShow(blogin){    
    ...
    if (tabtype == 'downIng') {
        ...
        if (nCnt <= 0 ) {
            iconHtml = '<div class="nocdImg"></div><p>所有下载任务已经完成~</p>';
            iconHtml += '<p><a href="javascript:;" onclick="JumpPageToFin();">快去听听吧</a></p>';
        }
...
function DownLoadTypeChg(){
    ...
    if (inner.indexOf('收藏夹') == 0) {
        ...
        $('.CD_Collections').show();
        $('.hr_sep_left').show();
        setDataToConfig('hificolDown', 'tabtype', 'collect');
    }
    SetPageIconShow(blogin);
}
...
function ShowCDDownloadTips(str){
    ...
    if( tipType != 'undonetasktoomany' && tipType != 'cachenotenough' && tipType != 'copyfilediskfull' ){
        return;
    }
    
    var strdisk = '';
    if( tipType == 'cachenotenough' || tipType == 'copyfilediskfull'){
