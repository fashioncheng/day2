...
function createPlaylistBlock (obj,from,isrefresh,someObj) {
    ...
    var musicNum = parseInt(obj.muiscnum)||parseInt(info);
    if(isNaN(musicNum)){
        musicNum=0;
    }
    if(musicNum == 0)return;
    if(disname==""){
        disname="my";
    }
    if(someObj){
    ...
    if(someObj && from!="index"){
        ...
        other = '|from=index|psrc=首页->' + someObj.psrc+"|csrc="+someObj.csrc;
    ...
    if(from=="myFavor"){
        ipsrc = '我的歌单->'+obj.name + '-<PID_'+obj.sourceid+';SEC_-1;POS_-1;DIGEST_8>';
        csrc = someObj.csrc+'->'+name;
...
function createArtistBlock(obj,from,isrefresh,type) {
    ...
    switch(from){
        case 'similar':
            ...
            var songnum = obj.songnum_total || obj.songnum;
...
function createGedanMusicList(obj,index,rn,pn,psrc) {
    ...
    arr[arr.length] = '</a><a data-rid="'+id+'" hidefocus href="###" class="newm_more" title="相似推荐"></a></div></div>';
...
function createOpenMusicList(obj,index,rn,pn,psrc,enter) {
    ...
    if(index<5){
        arr[arr.length] = '<li data-change="'+changenum+'" class="music_wrap ';
    }else{
        arr[arr.length] = '<li style="display:none;" data-change="'+changenum+'" class="music_wrap ';
    }
    arr[arr.length] = getCopyrightClass(obj);
    arr[arr.length] = '" c-rid="';
    arr[arr.length] = id;
...
function createfixedMusicList(obj,from,index,psrc){
    ...
    html[html.length] = '<a hidefocus href="###" class="m_down" style="margin-left:8px;" title="下载歌曲">';
